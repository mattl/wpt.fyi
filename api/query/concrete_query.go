...
type Binder interface {
	// Bind produces an executable Query and/or error after binding its inputs to
	// a query service mechanism. E.g., an in-memory cache may verify that the
	// given runs are in the cache and extract results data subsets that pertain
	// to the runs before producing a Query implementation that can operate over
	// the subsets directly.
	Bind([]shared.TestRun, AbstractQuery) (Query, error)
}

// Query is the executable form of a query that returns results and/or an error.
type Query interface {
	// Execute runs the query. The result set type depends on the underlying query
	// service mechanism that the Query was bound to.
